...
class DataStore:
    ...
    def get_player_dataframe(self, player_id: int = None, player_name: str = None) -> pd.DataFrame:
        if player_id is None:
            ...
            players = self._get_csv("Players.csv")
            player_id = players.loc[players["Name"] == player_name, "ID"].iloc[0]

        df = self._get_csv("PlayersGames.csv")
        df = df[df["Player"] == player_id]

        df_games = self._get_csv("Games.csv")
        df = df.merge(df_games, left_on="Game", right_on="ID").drop(columns="ID")

        df_matches_games = self._get_csv("MatchesGames.csv")
    ...
    def _get_csv(self, file_name: str) -> pd.DataFrame:
        if file_name not in self.data_cache:
            self.data_cache[file_name] = pd.read_csv(self.data_directory / file_name)
